...
def partial_match_score(truth: List[Rationale], pred: List[Rationale], thresholds: List[float]) -> List[Dict[str, Any]]:
    ...
    print(ann_to_rat.items())
    print(pred_to_rat.items())
...
def score_hard_rationale_predictions(truth: List[Rationale], pred: List[Rationale]) -> Dict[str, Dict[str, float]]:
    ...
    truth = set(truth)
    pred = set(pred)
    micro_prec = len(truth & pred) / (len(pred) if len(pred)>0 else 1)
    micro_rec = len(truth & pred) / (len(truth) if len(truth)>0 else 1)
    ...
    macro_prec = sum(instance['p'] for instance in instances_to_scores.values()) / (len(instances_to_scores) if len(instances_to_scores)>0 else 1)
    macro_rec = sum(instance['r'] for instance in instances_to_scores.values()) / (len(instances_to_scores) if len(instances_to_scores)>0 else 1)
    macro_f1 = sum(instance['f1'] for instance in instances_to_scores.values()) / (len(instances_to_scores) if len(instances_to_scores)>0 else 1)
...
def _has_hard_predictions(results: List[dict]) -> bool:
    ...
    (results[0])
    #print('rationales' in results[0])
    #print(len(results[0]['rationales']) > 0)
    #print('hard_rationale_predictions' in results[0]['rationales'][0])
    #print(results[0]['rationales'][0]['hard_rationale_predictions'] is not None)
    #print(len(results[0]['rationales'][0]['hard_rationale_predictions']) > 0)
    #print(results[0]['rationales'][0]['hard_rationale_predictions'])
